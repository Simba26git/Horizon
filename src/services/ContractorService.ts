...
export class ContractorService {
  ...
  async findMatchingContractors(requirements: ProjectRequirements): Promise<Contractor[]> {
    let query = supabase
      .from('contractors')
      .select('*')
      .eq('status', 'active');
    ...
    if (requirements.specializations.length > 0) {
      query = query.contains('specializations', requirements.specializations);
    }
    ...
    // Further filter and rank contractors
    return this.rankContractors(contractors || [], requirements);
  }
